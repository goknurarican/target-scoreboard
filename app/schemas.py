...
class EvidenceRef(BaseModel):
    ...
    source_quality: Optional[Literal["high", "medium", "low"]] = Field(None, description="Source quality assessment")
    category: Optional[Literal["literature", "databases", "vantai", "other"]] = Field(None,
                                                                                      description="Mapped evidence category")
...
class ModalityFitScores(BaseModel):
    ...
    dc_score: Optional[float] = Field(None, ge=0, le=1, description="ADC targeting potential")
...
class TargetScore(BaseModel):
    """Legacy target score model for backward compatibility."""
    target: str = Field(..., description="Target gene symbol")
    total_score: float = Field(..., ge=0, le=1, description="Combined weighted score")
    breakdown: TargetBreakdown = Field(..., description="Individual channel scores")
    evidence_refs: List[EvidenceRef] = Field(default_factory=list, description="Structured evidence references")
    evidence_refs_legacy: List[str] = Field(default_factory=list, description="Legacy string evidence refs")
    evidence_summary: Dict[str, int] = Field(default_factory=dict, description="Evidence count by category")
    total_evidence: int = Field(0, ge=0, description="Convenience count for FE badges")
    data_version: str = Field(..., description="Data version identifier")
    explanation: Dict[str, Any] = Field(default_factory=dict, description="Explanation object")
    timestamp: datetime = Field(default_factory=get_utc_now, description="Scoring timestamp")
    warnings: Optional[List[str]] = Field(None, description="Warning messages")
    channels: Optional[Dict[str, ChannelScore]] = Field(None, description="Full channel details")
