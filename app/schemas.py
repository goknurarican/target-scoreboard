...
"""
Pydantic schemas for VantAI Target Scoreboard API.
...
class EvidenceRef(BaseModel):
    """Clickable evidence reference with external links."""
...
class Explanation(BaseModel):
    ...
    evidence_refs: List[EvidenceRef] = Field(..., description="Clickable evidence references")
...
class TargetScore(BaseModel):
    ...
    evidence_refs: List[str] = Field(default_factory=list)          # <-- EKLE
    data_version: str = "Unknown"
    explanation: Dict[str, Any] = Field(default_factory=dict)       # <-- EKLE
    timestamp: datetime
